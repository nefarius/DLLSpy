                                        ...
                                        char* TerminatorString, long* OutputSize, string& sOutput) const
{
    ...
    strlenCharsOfInterest = static_cast<long>(strlen((char*)CharsOfInterest));
    strlenTerminatorString = static_cast<long>(strlen(TerminatorString));
    ...
    if (rc == 0)
    {
        OutputBuffer = static_cast<char*>(malloc(MYSTRINGS_MAXIMUM_FILE_SIZE));
        if (OutputBuffer)
        {
            OutputBufferPos = OutputBuffer;
            for (i = 0; i < static_cast<long>(SizeActuallyRead) - 1; i++)
            {
                // dbg
    ...
    if (OutputBuffer)
    {
        ...
        Size2realloc = static_cast<long>(OutputBufferPos - OutputBuffer);
        (*OutputSize) = Size2realloc;
        OutputBufferPos = OutputBuffer;
        OutputBuffer = static_cast<char*>(realloc(OutputBuffer, Size2realloc));
                                                ...
                                                DWORD* SizeActuallyRead) const
{
    ...
    (*FileBuffer) = static_cast<CHAR*>(malloc(SizeToRead + 2));
...
long StringsExtractor::ReadBinaryFileIntoMemory(WCHAR* CandidateFile, CHAR* FileBuffer, DWORD SizeOfBuffer,
                                                DWORD* SizeActuallyRead)
{
    DWORD rc = 0;
    HANDLE hFileToRead;
    ...
    *SizeActuallyRead = 0;
    ...
    hFileToRead = CreateFileW(CandidateFile, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,
    ...
    if (ReadFile(hFileToRead, FileBuffer, SizeToRead, &dwBytesRead, nullptr) == FALSE)
    {
        dwBytesRead = 0;
    }
...
long StringsExtractor::MyGetFileSize(FILE* f)
{
    ...
    FileSize = static_cast<DWORD>(ftell(f)); // get current file pointer
